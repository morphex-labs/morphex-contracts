...
async function getFtmValues() {
  ...
  const sender = new ethers.Wallet(FTM_DEPLOY_KEY).connect(provider)
  ...
  return { sender, transfers, totalTransferAmount, tokens, gasToken: "FTM" }
}

async function getTestnetFtmValues() {
  const provider = new ethers.providers.JsonRpcProvider(FTM_TESTNET_URL)
  const sender = new ethers.Wallet(FTM_TESTNET_DEPLOY_KEY).connect(provider)
